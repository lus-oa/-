...
4.假设一台处理机一个时刻只允许加工一个任务，一个任务必须连续加工完成。给定任务集合 $\ T= \lbrace T_{1},T_{2},...,T_{n} \rbrace \$，任务之间无先后关系。所有任务加工的最早起始时间为： $\ r_{i}=0 , 1 \leq i \leq n\$ ，限定 $\ T_{i}\$ 的最迟完成时间为 $\ d_{i}\in Z^+\$ ，加工时间长度Li∈Z+，1≤i≤n。试设计算法计算任务排工表(Ti)，使按时加工完成的任务数达到最大。
