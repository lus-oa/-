...
4.假设一台处理机一个时刻只允许加工一个任务，一个任务必须连续加工完成。给定任务集合 $\T={T_{1},T_{2},...,T_{n}}\$，任务之间无先后关系。所有任务加工的最早起始时间为：ri = 0，1≤i≤n，限定Ti 的最迟完成时间为di∈Z+，加工时间长度Li∈Z+，1≤i≤n。试设计算法计算任务排工表(Ti)，使按时加工完成的任务数达到最大。
